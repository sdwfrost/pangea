...
tr.fn <- tr.fn[grep("281014",tr.fn)]
p.fn <- list.files("./",pattern="\\.p$")
p.fn <- p.fn[grep("281014",p.fn)]
...
for(i in 1:numtreefiles){
  print(i)
  cmd <- getmcc("keep",0,0.0,paste(tr.fn[i],".time",sep=""),paste(tr.fn[i],".time.mcc",sep=""),"/Users/simon/Programs/BEASTv1.8.1/bin/treeannotator")
  system(cmd)
}
```
